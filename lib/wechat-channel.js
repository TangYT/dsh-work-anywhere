...
import { createInteractionBridge } from './interactions.js'
...
function log(...a) { console.log('[wechat-channel]', new Date().toISOString(), ...a) }
const sleep = (ms) => new Promise((r) => setTimeout(r, ms))

/**
 * 连续轮询失败的日志/重试策略（纯函数，便于测试）：
 * - failures 1-3：输出详情日志，2s/2s/30s 退避（failures≥3 后固定 30s）
 * - failures 4+：静默，每 10 次（10/20/…）输出一条摘要，避免瞬时抖动刷屏
 * - failures 归零：若此前曾持续失败 ≥5 次，输出一条恢复提示
 * @returns {{log:'detail'|'tick'|'silent'|'recovered', delayMs:number}}
 */
export function failurePolicy(failures, prevFailures) {
  if (failures <= 0) {
    return prevFailures >= 5 ? { log: 'recovered', delayMs: 0 } : { log: 'silent', delayMs: 0 }
  }
  const delayMs = failures >= 3 ? 30000 : 2000
  if (failures <= 3) return { log: 'detail', delayMs }
  if (failures % 10 === 0) return { log: 'tick', delayMs }
  return { log: 'silent', delayMs }
}

/** 把 fetch/传输错误拼成可诊断日志（含 cause 的 code/message/syscall）。 */
export function formatErrorDetail(e) {
  let out = String(e && e.message || e)
  const c = e && e.cause
  if (c) {
    const parts = []
    ...
    if (parts.length) out += ' | cause: ' + parts.join(' ')
  }
  return out
}
...
export function apply(ctx) {
  ...
  async function monitorLoop(signal) {
    let failures = 0
    // 失败登记：返回下一次重试等待毫秒；日志按 failurePolicy 节流（前 3 次详情 → 每 10 次摘要）
    const registerFailure = (detail) => {
      const prev = failures
      failures += 1
      const p = failurePolicy(failures, prev)
      if (p.log === 'detail') log('getupdates error:', detail)
      else if (p.log === 'tick') log('getupdates 持续失败 ' + failures + ' 次（30s 自动重试中），最近: ' + String(detail).slice(0, 200))
      return p.delayMs
    }
    const registerSuccess = () => {
      const prev = failures
      failures = 0
      const p = failurePolicy(0, prev)
      if (p.log === 'recovered') log('getupdates 已恢复（此前连续失败 ' + prev + ' 次）')
    }
    while (!signal.aborted && state.connected) {
      try {
        ...
        if (errcode !== null || ret !== null) {
          ...
          await sleep(registerFailure('api ' + JSON.stringify(resp).slice(0, 200)))
          continue
        }
        registerSuccess()
        ...
        if (signal.aborted) return
        await sleep(registerFailure(formatErrorDetail(e)))
      }
    }
  }
  ...
  async function handleInbound(msg) {
    ...
    // 交互桥等待器：该对端有挂起的审批/问答时，消息交付给等待器（绝不进入 Agent）
    if (interactionBridge && await interactionBridge.deliver(peerId, text)) return
  ...
  // ---------- 微信交互桥（审批 / 选项问答 / 计划评审 → 转发微信并交还 Web answerer） ----------
  const interactionBridge = createInteractionBridge({
    ctx, log,
  ...
  ctx.effect(() => () => {
    ...
    try { interactionBridge.stop() } catch {}
