...
export function createInteractionBridge(deps) {
  ...
  // ---------- WebSocket 下行通道（/api/events.mux 是 WS-only 端点，普通 GET 返回 426） ----------
  // 每帧 = {rpcId, method: <type>, payload: {...}} 的 JSON 文本（下行单向，客户端不得发消息）；
  // 连接即重放全部 pending；断开后指数退避重连（日志节流，避免刷屏）。
  function handleWsMessage(raw) {
    let msg
    try { msg = JSON.parse(String(raw)) } catch { return }
    if (!msg || typeof msg.method !== 'string' || typeof msg.rpcId !== 'string') return
    // 归一化为与进程内 SSE 相同的形状：type 放回 payload
    handleFrame({ rpcId: msg.rpcId, payload: { type: msg.method, ...(msg.payload || {}) } })
  }
  async function bridgeLoop(signal) {
    ...
    while (!signal.aborted) {
      ...
      await new Promise((resolve) => {
        ...
        const abortOnSignal = () => { try { ws.close() } catch {}; finish() }
        if (signal.aborted) { abortOnSignal(); return }
        signal.addEventListener('abort', abortOnSignal, { once: true })
        ws.onopen = () => { failures = 0 }
        ws.onmessage = (ev) => {
          try { handleWsMessage(ev.data) } catch (e) { log('interaction bridge ws message error:', e && e.message) }
        }
        ws.onerror = () => {}
        ws.onclose = () => {
          try { signal.removeEventListener('abort', abortOnSignal) } catch {}
          finish()
        }
      })
      if (signal.aborted) return
      failures++
      const backoff = Math.min(1500 * Math.pow(2, Math.min(failures - 1, 5)), 60000)
      if (failures <= 3 || failures % 10 === 0) log('interaction bridge mux reconnect #' + failures + ' in ' + (backoff / 1000) + 's')
      await sleep(backoff)
    }
  }
  function start() {
